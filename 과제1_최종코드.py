...
# ----------------------------------------------------
    ...
    if abs(d) > MAP_Y1 : d = float('inf')
    elif abs(d) < 1/MAP_X1 : d = 0
...
# 새로운 node에 대해서 장애물을 업데이트 하는 함수
    ...
    if new_node.parent.incircle == new_node.incircle == False and new_node.parent.step == new_node.step:
...
# ----------------------------------------------------
    ...
    while openList:
        ...
        steps = [(0, -SINGLE_STEP), (0, SINGLE_STEP), (-SINGLE_STEP, 0), (SINGLE_STEP, 0), (SINGLE_STEP/2,SINGLE_STEP/2),(-SINGLE_STEP/2,SINGLE_STEP/2), (SINGLE_STEP/2,-SINGLE_STEP/2), (-SINGLE_STEP/2,-SINGLE_STEP/2)]
        ...
        for step in steps:
            ...
            if nodePosition[0] < MAP_X0 or nodePosition[1] < MAP_Y0 or nodePosition[0] > MAP_X1 or nodePosition[1] > MAP_Y1: 
                continue	
            new_node = Node(currentNode, nodePosition)
            #다음 노드의 포지션이 현재 장애물 안에 있으면 탐색 중지
            min_distance = min(distance(new_node.position, obstacle) for obstacle in currentNode.obstacles)
            ...
            if min(distance(new_node.position, obstacle) for obstacle in endNode.obstacles) <= R-SINGLE_STEP/2:
                #endNode의 장애물로 진행하는 step 1번과 그렇지 않은 step 3번에 대해 똑같은 g값을 갖게하여 
...
# ----------------------------------------------------
    ...
    elif abs(forward_distance-backward_distance) < Lf:
        return velocity
    # 속도의 부호를 반전시킨다.
    else:
        return -1*velocity
